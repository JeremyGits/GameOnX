// System/main.js (further updated paths for consistency, added art to CSP img-src, minor logging tweaks)
...
let splashWindow, loginWindow, mainWindow, onboardingWindow, pinWindow, createRoomWindow;
...
function createSplashWindow() {
  ...
  splashWindow.loadFile(path.join(__dirname, "../Assets/views/splash.html")); // Updated to use path.join for cross-platform safety
...
function createLoginWindow() {
  ...
  loginWindow = new BrowserWindow({
    ...
    height: 400,
    webPreferences: {
      preload: path.join(__dirname, "preload.js"), // Relative to System
      nodeIntegration: false,
      contextIsolation: true,
      sandbox: false,
      contentSecurityPolicy: "default-src 'self'; script-src 'self' 'unsafe-inline'; connect-src 'self' http://localhost:* ws://localhost:*; style-src 'self' 'unsafe-inline'; img-src 'self' data: file: ../Assets/art/*" // Added file: and art path for BMP theme images
    },
  });
  loginWindow.loadFile(path.join(__dirname, "../Assets/views/login.html")).then(() => logToFile("login.html loaded")).catch(err => logToFile(`login.html load error: ${err.message}`));
...
function createMainWindow(games) {
  ...
  mainWindow = new BrowserWindow({
    ...
    webPreferences: {
      ...
      contextIsolation: true,
      sandbox: false,
      contentSecurityPolicy: "default-src 'self'; script-src 'self' 'unsafe-inline'; connect-src 'self' http://localhost:* ws://localhost:*; style-src 'self' 'unsafe-inline'; img-src 'self' data: file: ../Assets/art/*" // Updated img-src for art/themes
    },
  });
  mainWindow.loadFile(path.join(__dirname, "../Assets/views/index.html")).then(() => logToFile("index.html loaded")).catch(err => logToFile(`index.html load error: ${err.message}`));
...
function createOnboardingWindow() {
  onboardingWindow = new BrowserWindow({
    ...
    webPreferences: {
      ...
      contextIsolation: true,
      sandbox: false,
      contentSecurityPolicy: "default-src 'self'; script-src 'self' 'unsafe-inline'; connect-src 'self' http://localhost:* ws://localhost:*; style-src 'self' 'unsafe-inline'; img-src 'self' data: file: ../Assets/art/*"
    },
  });
  onboardingWindow.loadFile(path.join(__dirname, "../Assets/views/onboarding.html")).then(() => logToFile("onboarding.html loaded")).catch(err => logToFile(`onboarding.html load error: ${err.message}`));
...
function createPinWindow(oauth_token) {
  ...
  pinWindow.loadFile(path.join(__dirname, "../Assets/views/pin.html")).then(() => logToFile("pin.html loaded")).catch(err => logToFile(`pin.html load error: ${err.message}`));
...
ipcMain.on("delete-user", (event, id) => {
  logToFile(`Deleting user with id: ${id}`);
  let users = store.get("users") || [];
  const index = users.findIndex(u => u.id_str === id);
  if (index !== -1) {
    const user = users.splice(index, 1)[0];
    store.set("users", users);
    if (store.get("currentUser")?.id_str === id) {
      store.delete("currentUser");
    }
    // Optionally delete from backend
    axios.delete(`http://localhost:3000/user/${id}`).then(() => logToFile(`Backend user deleted: ${id}`)).catch(err => {
      console.error("Backend delete error:", err);
      logToFile(`Backend delete error: ${err.message}`);
    });
    // Refresh profiles in login window
    if (loginWindow) {
      loginWindow.webContents.send("user-profiles", users);
      logToFile("User profiles refreshed after delete");
    }
  } else {
    logToFile(`User not found for delete: ${id}`);
  }
});
...
ipcMain.on("launch-game", (event, gamePath) => {
  child_process.exec(gamePath || "notepad.exe", (err) => {
    if (err) {
